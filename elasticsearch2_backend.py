...
try:
    ...
    if elasticsearch.__version__[0] == 1:
        FACETS = 'facets'
    ...
    raise MissingDependency(
        "The 'elasticsearch' backend requires the installation "
        "of 'elasticsearch'. Please refer to the documentation."
    )
...
class ElasticsearchSearchBackend(BaseSearchBackend):
    ...
    def __init__(self, connection_alias, **connection_options):
        ...
        if 'INDEX_NAME' not in connection_options:
            raise ImproperlyConfigured(
                "You must specify a 'INDEX_NAME' in "
                "your settings for connection '%s'." % connection_alias
            )

        # Create the elasticsearch connection given
    ...
    def build_search_kwargs(self, query_string, sort_by=None, start_offset=0, end_offset=None,
        ...
        if query_string == '*:*':
            ...
            if elasticsearch.__version__[0] < 2:
                kwargs['query']['query_string']['fuzzy_min_sim'] = FUZZY_MIN_SIM
        ...
        if highlight:
            ...
            kwargs['highlight'] = {
                'fields': {
                    content_field: {'store': 'yes'} if elasticsearch.__version__[0] < 2 else {},
                }
            }
                ...
                if 'facet_filter' in extra_options:
                    if elasticsearch.__version__[0] < 2:
                        ...
                        facet = {
                            'filter1': {
                                'aggs': facet_options,
                                'filter': extra_options.pop('facet_filter')
                            }
                        }
                        if 'order' in extra_options:
                            val = extra_options['order']
                            if 'reverse' in val:
                                extra_options['order'] = {'_{}'.format(val.split('_')[1]): 'asc'}
                            else:
                                extra_options['order'] = {'_{}'.format(val): 'desc'}
                        facet_options['terms'].update(extra_options)
                        kwargs[FACETS][facet_fieldname] = facet
                else:
                    if elasticsearch.__version__[0] > 1 and 'order' in extra_options:
                        val = extra_options['order']
                        if 'reverse' in val:
                            extra_options['order'] = {'_{}'.format(val.split('_')[1]): 'asc'}
                        else:
        ...
        if date_facets is not None:
            ...
            for facet_fieldname, value in date_facets.items():
                ...
                if elasticsearch.__version__[0] < 2:
                    kwargs[FACETS][facet_fieldname]['facet_filter'] = {
                        "range": {
                            facet_fieldname: {
                    ...
                    kwargs[FACETS] = {
                        'filter1': {
                            'aggs': {
                                facet_fieldname: kwargs[FACETS].pop(facet_fieldname)
                            },
                            'filter': {
                                'range': {
                                    facet_fieldname: {
        ...
        if filters:
            kwargs["query"] = {FILTERED: {FILTERED_QUERY: kwargs.pop("query")}}
            if len(filters) == 1:
    ...
    def more_like_this(self, model_instance, additional_query_string=None,
        ...
        try:
            if elasticsearch.__version__[0] < 2:
                ...
                doc = self.conn.get(index=self.index_name, doc_type='modelresult', id=doc_id)
                query = {
                    'query': {
                        'more_like_this': {
                            'fields': [field_name],
                            'like': doc['_source'].get(field_name)
                        }
                    }
                }
    ...
    def build_schema(self, fields):
        content_field_name = ''
        mapping = {
            DJANGO_CT: {'type': STRING, 'index': INDEXED, 'include_in_all': False},
            DJANGO_ID: {'type': STRING, 'index': INDEXED, 'include_in_all': False},
        }
